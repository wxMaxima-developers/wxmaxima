...
// Some drawing contents
  ...
  while (region) {
    ...
    if ((m_hCaretActive) && (m_hCaretPositionStart == NULL) &&
      ...
      int caretY = ((int)m_configuration->GetGroupSkip()) / 2 +
...
wxString Worksheet::ConvertSelectionToMathML() {
  ...
  if (s.Length() < 1000000) {
    ...
    if (doc.IsOk()) {
      ...
      doc.Save(ostream);
      s = wxString::FromUTF8(
          (char *)ostream.GetOutputStreamBuffer()->GetBufferStart(),
    ...
    if (m_blinkDisplayCaret) {
      ...
      if (GetActiveCell()) {
        ...
        if (!m_hCaretPosition) {
          ...
          int caretY =
              ((int)m_configuration->GetGroupSkip()) / 2 + rect.GetBottom() + 1;
  ...
  if (CanCopy(true)) {
    ...
    if (wxTheClipboard->IsUsingPrimarySelection()) {
      ...
      if (wxTheClipboard->Open()) {
        ...
        wxLogMessage(wxString::Format(_("Middle-click clipboard data: %s"),
                                      data.c_str()));
