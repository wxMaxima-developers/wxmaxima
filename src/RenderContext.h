...
/*! The state of the current worksheet render pass.
  ...
  worksheet or the cells. Copying a RenderContext (for printing, the diff
  viewer etc.) copies the geometry and the recalc-DC pointer but neither the
  owned worksheet client DC (the copy doesn't own the window) nor a running
  layout deadline.
*/
class RenderContext
{
