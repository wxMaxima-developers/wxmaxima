...
void SubSupCell::Draw(wxPoint point, wxDC *dc, wxDC *antialiassingDC) {
  ...
  if (DrawThisCell(point)) {
    ...
    if (m_preSubCell) {
      ...
      presub.y += static_cast<long>(m_baseCell->GetMaxDrop()) + m_preSubCell->GetCenterList() -
	.8 * m_fontSize_Scaled + static_cast<long>(MC_EXP_INDENT);
	...
	m_preSupCell->GetCenterList() - static_cast<long>(.8 * m_fontSize_Scaled) +
	MC_EXP_INDENT;
