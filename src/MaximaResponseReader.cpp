...
void MaximaResponseReader::ReadPrompt(const wxString &data) {
  ...
  if (label.StartsWith(wxS("MAXIMA>")) || label.StartsWith("(dbm:")) {
    ...
    m_wxMaxima.m_configuration.InLispMode(true);
    // Status-bar pictogram for these special REPL modes. Set it LAST so it
    // wins over the userinput / main-prompt status the branches above set for
    // this prompt; a subsequent normal "(%iN)" prompt (e.g. after :top /
    // :continue leaves the debugger, or (to-maxima) leaves Lisp mode) sets a
    // non-special status and clears it. The debugger (dbm) wins over plain
    // Lisp mode (MAXIMA>).
    if (label.StartsWith(wxS("(dbm:")))
      m_wxMaxima.StatusMaximaBusy(StatusBar::MaximaStatus::debugging);
    else
      m_wxMaxima.StatusMaximaBusy(StatusBar::MaximaStatus::lispmode);
  } else {
    if (m_wxMaxima.m_configuration.InLispMode())
      wxLogMessage(_("Ended lisp mode after receiving a maxima prompt!"));
    m_wxMaxima.m_configuration.InLispMode(false);
...
void MaximaResponseReader::EnterLdbMode() {
...
void MaximaResponseReader::ReadStdErr() {
  ...
  if (m_wxMaxima.m_maximaProcess->IsInputAvailable()) {
    ...
    // sbcl prints the LDB banner on stdout when the Lisp runtime drops into
    // its low-level debugger. (Note: stdout is block-buffered, so the banner is
    // often stuck in sbcl's buffer and never reaches us - the stderr prompt
    // below is the reliable trigger. We still key on the banner when it does
    // arrive.)
    if (LdbSupport::ContainsLdbBanner(o))
      EnterLdbMode();
