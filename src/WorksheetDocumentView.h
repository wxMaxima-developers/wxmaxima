...
/*! \file
  ...
  to tell the view to re-lay-out, redraw and remember that the file is now
  unsaved. Rather than let WorksheetDocument depend on the whole Worksheet, it
  talks back through this small one-way interface, which Worksheet implements by
  forwarding to its real layout/redraw/save machinery. All notifications; no
...
public:
  ...
  //! The document's content changed: it is no longer in its saved state.
  virtual void NotifyDocumentModified() = 0;
};
