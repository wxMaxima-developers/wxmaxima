...
struct {
  wxMaximaFrame::Event id;
  wxString name;
} static const PaneNames[]{
  {wxMaximaFrame::menu_pane_math, wxT("math")},
  {wxMaximaFrame::menu_pane_history, wxT("history")},
  {wxMaximaFrame::menu_pane_structure, wxT("structure")},
  {wxMaximaFrame::menu_pane_xmlInspector, wxT("XmlInspector")},
  {wxMaximaFrame::menu_pane_format, wxT("format")},
  {wxMaximaFrame::menu_pane_greek, wxT("greek")},
  {wxMaximaFrame::menu_pane_unicode, wxT("unicode")},
  {wxMaximaFrame::menu_pane_log, wxT("log")},
  {wxMaximaFrame::menu_pane_variables, wxT("variables")},
  {wxMaximaFrame::menu_pane_draw, wxT("draw")},
  {wxMaximaFrame::menu_pane_symbols, wxT("symbols")},
  {wxMaximaFrame::menu_pane_stats, wxT("stats")},

  {wxMaximaFrame::named_pane_toolbar, wxT("toolbar")},
  {wxMaximaFrame::named_pane_console, wxT("console")},
  }; // 12 menu panes + 2 named panes

wxAuiPaneInfo &SetBasicPane(wxAuiPaneInfo &info)
{
  return
    info.TopDockable(true).BottomDockable(true).LeftDockable(true).RightDockable(true).
    PaneBorder(true);
}

wxAuiPaneInfo &SetBasicBtnPane(wxAuiPaneInfo &info)
{
  return
    info.TopDockable(true).BottomDockable(true).LeftDockable(true).RightDockable(true).
    PaneBorder(true).CloseButton(true).PinButton(true);
}

void wxMaximaFrame::SetupPanes()
{
  wxPanel *statPane = CreateStatPane();
  ...
  wxPanel *variablesPanel = new wxPanel(this,wxID_ANY);
  wxEventBlocker variablesBlocker(variablesPanel);
  m_worksheet->m_variablesPane = new Variablespane(variablesPanel, wxID_ANY);
  wxSizer *variablesSizer = new wxBoxSizer(wxVERTICAL);
  variablesSizer->Add(m_worksheet->m_variablesPane,wxSizerFlags().Expand());
  variablesPanel->SetSizerAndFit(variablesSizer);

  m_symbolsPane = new SymbolsPane(this, m_worksheet->m_configuration, m_worksheet);
  wxEventBlocker symbolsBlocker(m_symbolsPane);

  m_drawPane = new DrawPane(this, -1);
  ...
  struct PaneDefinition {
    wxWindow *panel;
    Event id;
    wxAuiPaneInfo (*infoSetter)(wxAuiPaneInfo &, wxWindow *panel);
    wxAuiManagerDock dockDirection;
  };

  GetPane(menu_pane_unicode).
    Show(false).Gripper(false).CloseButton(true).PinButton(true);
  GetPane(menu_pane_variables).
    Gripper(false).CloseButton(true).PinButton(true);
  GetPane(menu_pane_log).
    Gripper(false).CloseButton(true).PinButton(true);
  GetPane(menu_pane_symbols).
    Show(true).Gripper(false).CloseButton(true);
  GetPane(menu_pane_draw).
    Show(true).CloseButton(true).Gripper(false);

  PaneDefinition panes[]{
    { m_history, menu_pane_history, [](auto &i, auto){
       return SetBasicBtnPane(i); }, wxAUI_DOCK_RIGHT },
    { m_worksheet->m_tableOfContents, menu_pane_structure, [](auto &i, auto){
       return SetBasicBtnPane(i); }, wxAUI_DOCK_RIGHT },
    { m_xmlInspector, menu_pane_xmlInspector, [](auto &i, auto){
       return SetBasicBtnPane(i); }, wxAUI_DOCK_RIGHT },
    { statPane, menu_pane_stats, [](auto &i, auto){
       return SetBasicBtnPane(i); }, wxAUI_DOCK_LEFT },
    { greekPane, menu_pane_greek,
     [](auto &i, auto p){ return
                             SetBasicBtnPane(i).DockFixed(false).Gripper(false).
                             FloatingSize(p->GetEffectiveMinSize()); }, wxAUI_DOCK_LEFT },
    { unicodePane, menu_pane_unicode,
     [](auto &i, auto p){ return SetBasicBtnPane(i).DockFixed(false).Gripper(false).
                             FloatingSize(p->GetEffectiveMinSize()); }, wxAUI_DOCK_LEFT },
    { m_logPane, menu_pane_log,
     [](auto &i, auto p){ return SetBasicBtnPane(i).Gripper(false).
                             /*.MinSize(m_logPane->GetEffectiveMinSize())*/
                             /*.BestSize(m_logPane->GetEffectiveMinSize())*/
                             FloatingSize(p->GetEffectiveMinSize()); }, wxAUI_DOCK_LEFT },
    { variablesPanel, menu_pane_variables,
     [](auto &i, auto p){ return SetBasicBtnPane(i).DockFixed(false).
                             Gripper(false).FloatingSize(p->GetEffectiveMinSize()); },
     wxAUI_DOCK_BOTTOM },
    { m_symbolsPane, menu_pane_symbols,
     [](auto &i, auto p){ return SetBasicBtnPane(i).CloseButton(true).DockFixed(false).
                             Gripper(false).FloatingSize(p->GetEffectiveMinSize()); },
     wxAUI_DOCK_BOTTOM },
    { CreateMathPane(), menu_pane_math,
     [](auto &i, auto){ return SetBasicPane(i).CloseButton(true); }, wxAUI_DOCK_LEFT },
    { CreateFormatPane(), menu_pane_format,
     [](auto &i, auto){ return SetBasicPane(i).CloseButton(true); }, wxAUI_DOCK_LEFT },
    { m_drawPane, menu_pane_draw,
     [](auto &i, auto){ return SetBasicPane(i).CloseButton(true); }, wxAUI_DOCK_LEFT },

    { m_worksheet->m_mainToolBar, named_pane_toolbar,
     [](auto &i, auto){ return
                           SetBasicPane(i).Top()./*ToolbarPane().*/PaneBorder(false).
                           CaptionVisible(false).CloseButton(false).
                           LeftDockable(false).RightDockable(false).
                           DockFixed().Gripper(false).Row(1); }},
    { m_worksheet, named_pane_console,
     [](auto &i, auto){ return SetBasicPane(i).
                           Center().CloseButton(false).CaptionVisible(false).
                           MinSize(wxSize(100,100)).PaneBorder(false).Row(2); }},
    };
  for (auto &p : panes)
  {
    wxAuiPaneInfo info;
    info.dock_direction = p.dockDirection;
    m_manager.AddPane(p.panel, p.infoSetter(info, p.panel).Name(GetPaneName(p.id)));
  }
  ...
  for (Event id :
       {menu_pane_xmlInspector, menu_pane_stats, menu_pane_greek, menu_pane_variables,
        menu_pane_math, menu_pane_format, menu_pane_log})
    GetPane(id).Show(false);
  ...
  GetPane(named_pane_console) = consolePane.info.Show(true);
...
const wxString &wxMaximaFrame::GetPaneName(Event id)
{
  for (auto &p : PaneNames)
    if (p.id == id) return p.name;
  wxASSERT(false);
}

wxAuiPaneInfo &wxMaximaFrame::GetPane(Event id)
{
  return m_manager.GetPane(GetPaneName(id));
}

bool wxMaximaFrame::IsPaneDisplayed(Event id)
{
  return GetPane(id).IsShown();
}
...
void wxMaximaFrame::ShowPane(Event id, bool show)
{
  ...
  case menu_pane_hideall:
    for (auto &pane : PaneNames)
      if (pane.id != menu_pane_draw && pane.id < named_pane_toolbar)
        m_manager.GetPane(pane.name).Show(false);
    ShowToolBar(false);
    break;
  case menu_pane_structure:
    m_worksheet->m_tableOfContents->UpdateTableOfContents(
      m_worksheet->GetTree(), m_worksheet->GetHCaret());
    /* fallthrough */
  default:
    GetPane(id).Show(show);
  }
  m_manager.Update();
}
...
{
  struct { Event event; const wxString &label; }
  static const buttonDefs[]{
    {button_ratsimp, _("Simplify")},
    {button_radcan, _("Simplify (r)")},
    {button_factor, _("Factor")},
    {button_expand, _("Expand")},
    {button_rectform, _("Rectform")},
    {button_subst, _("Subst...")},
    {button_trigrat, _("Canonical (tr)")},
    {button_trigsimp, _("Simplify (tr)")},
    {button_trigexpand, _("Expand (tr)")},
    {button_trigreduce, _("Reduce (tr)")},
    {button_solve, _("Solve...")},
    {button_solve_ode, _("Solve ODE...")},
    {button_diff, _("Diff...")},
    {button_integrate, _("Integrate...")},
    {button_limit, _("Limit...")},
    {button_taylor, _("Series...")},
    {button_plot2, _("Plot 2D...")},
    {button_plot3, _("Plot 3D...")},
  };
  ...
  for (auto &def : buttonDefs)
    grid->Add(new wxButton(panel, def.event, def.label,
                           wxDefaultPosition, wxDefaultSize,
                           wxBORDER_NONE | wxBU_EXACTFIT), 0, wxALL | wxEXPAND);
...
wxPanel *wxMaximaFrame::CreateStatPane()
{
  ...
  struct { wxSizer *sizer = {}; Event event = {}; const wxString &label = empty; }
  static const buttonDefs[]{
    {grid1, menu_stats_mean, _("Mean...")},
    {grid1, menu_stats_median, _("Median...")},
    {grid1, menu_stats_var, _("Variance...")},
    {grid1, menu_stats_dev, _("Deviation...")},
    {box1,  menu_stats_tt1, _("Mean Test...")},
    {box1,  menu_stats_tt2, _("Mean Difference Test...")},
    {box1,  menu_stats_tnorm, _("Normality Test...")},
    {box1,  menu_stats_linreg, _("Linear Regression...")},
    {box1,  menu_stats_lsquares, _("Least Squares Fit...")},
    {grid2, menu_stats_histogram, _("Histogram...")},
    {grid2, menu_stats_scatterplot, _("Scatterplot...")},
    {grid2, menu_stats_barsplot, _("Barsplot...")},
    {grid2, menu_stats_piechart, _("Piechart...")},
    {grid2, menu_stats_boxplot, _("Boxplot...")},
    {grid3, menu_stats_readm, _("Read Matrix...")},
    {grid3, menu_stats_enterm, _("Enter Matrix...")},
    {box3, menu_stats_subsample, _("Subsample...")},
    {} // needed to add box3 to the top box
  };
  ...
  {
    if (def.sizer)
      def.sizer->Add(new wxButton(panel, def.event, def.label,
                                  wxDefaultPosition, wxDefaultSize,
                                  wxBORDER_NONE | wxBU_EXACTFIT), 0, style, border);
    if (prevSizer && def.sizer != prevSizer)
      box->Add(prevSizer, 0, style, sizerBorder);
    prevSizer = def.sizer;
  }
...
{
  struct { Event event; const wxString &description; } const static buttonDefs[]{
    {menu_format_text, _("Text")},
    {menu_format_title, _("Title")},
    {menu_format_section, _("Section")},
    {menu_format_subsection, _("Subsection")},
    {menu_format_subsubsection, _("Subsubsection")},
    {menu_format_heading5, _("Heading 5")},
    {menu_format_heading6, _("Heading 6")},
    {menu_format_image, _("Image")},
    {menu_format_pagebreak, _("Pagebreak")},
    };
  ...
  for (auto &def : buttonDefs)
    grid->Add(new wxButton(panel, def.event, def.description,
...
void wxMaximaFrame::DrawPane::SetDimensions(int dimensions)
{
  ...
  m_draw_setup2d->Enable(dimensions <= 0);
  m_draw_contour->Enable(                   dimensions > 2);
  m_draw_setup3d->Enable(dimensions <= 0 || dimensions > 2);
...
{
  static wxButton *AddSpacer;
  struct {
    wxButton *&button;
    Event event;
    const wxString &label = CharButton::Definition::empty;
    const wxString &tooltip = CharButton::Definition::empty;
  } const static buttonDefs[]
    {
      {m_draw_setup2d, menu_draw_2d, _("2D"), _("Setup a 2D plot")},
      {m_draw_setup3d, menu_draw_3d, _("3D"), _("Setup a 3D plot")},
      //{AddSpacer},
      //{AddSpacer},
      {m_draw_explicit, menu_draw_explicit, _("Expression"),
       _("The standard plot command: Plot an equation as a curve")},
      {m_draw_implicit, menu_draw_implicit, _("Implicit Plot")},
      {m_draw_parametric, menu_draw_parametric, _("Parametric Plot")},
      {m_draw_points, menu_draw_points, _("Points")},
      //{AddSpacer},
      //{AddSpacer},
      {m_draw_title, menu_draw_title, _("Diagram title"), _("The diagram title")},
      {m_draw_axis, menu_draw_axis, _("Axis"), _("Setup the axis")},
      {m_draw_contour, menu_draw_contour, _("Contour"), _("Contour lines for 3d plots")},
      {m_draw_key, menu_draw_key, _("Plot name"), _("The next plot's title")},
      {m_draw_fgcolor, menu_draw_fgcolor, _("Line color"), _("The color of the next line to draw")},
      {m_draw_fillcolor, menu_draw_fillcolor, _("Fill color"), _("The fill color for the next objects")},
      {m_draw_grid, menu_draw_grid, _("Grid"), _("The grid in the background of the diagram")},
      {m_draw_accuracy, menu_draw_accuracy, _("Accuracy"), _("The Accuracy versus speed tradeoff")}
    };
  ...
  for (auto &def : buttonDefs)
  {
    if (&def.button == &AddSpacer)
      grid->AddSpacer(1);
    else
    {
      def.button = new wxButton(this, def.event, def.label, wxDefaultPosition, wxDefaultSize,
                                wxBORDER_NONE | wxBU_EXACTFIT);
      if (!def.tooltip.IsEmpty())
        def.button->SetToolTip(def.tooltip);
      grid->Add(def.button, 0, wxALL | wxEXPAND);
    }
  }
