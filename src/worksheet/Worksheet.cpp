...
bool Worksheet::RedrawIfRequested() {
  ...
  if (m_mouseMotionWas) {
    ...
    if (!GetViewCellPointers().GetGroupCellUnderPointer() ||
        (m_pointer_y <
         GetViewCellPointers().GetGroupCellUnderPointer()->GetRect().GetTop()) ||
        (m_pointer_y >
         GetViewCellPointers().GetGroupCellUnderPointer()->GetRect().GetBottom())) {
      const GroupCell *oldGroupCellUnderPointer =
        GetViewCellPointers().GetGroupCellUnderPointer();
      ...
      GroupCell *startSearch = GetViewCellPointers().GetGroupCellUnderPointer();
      ...
      } else if (!GetViewCellPointers().GetGroupCellUnderPointer()) {
      ...
      if ((m_configuration->HideBrackets()) &&
        ...
        if (GetViewCellPointers().GetGroupCellUnderPointer()) {
          RequestRedraw(wxRect(
                               0, GetViewCellPointers().GetGroupCellUnderPointer()->GetRect().GetTop(),
                               m_configuration->GetIndent(),
                               GetViewCellPointers().GetGroupCellUnderPointer()->GetRect().GetHeight()));
        }
      }
    }
    ...
    if (GetViewCellPointers().GetGroupCellUnderPointer()) {
      ...
      if (GetViewCellPointers().GetGroupCellUnderPointer()->GetOutputRect().Contains(
...
void Worksheet::ScrollToError() {
  GroupCell *errorCell = GetDocumentCellPointers().GetErrorList().LastError();
...
GroupCell *Worksheet::GetWorkingGroup(bool resortToLast) const {
  GroupCell *cell = GetDocumentCellPointers().GetWorkingGroup(resortToLast);

  if (!resortToLast)
    return cell;

  // The last group maxima was working on no more exists or has been deleted.
  if (!cell && GetHCaretCursor().IsActive())
    cell = GetHCaretCursor().Position();
...
void Worksheet::OnSize(wxSizeEvent &event) {
  ...
  Cell *CellToScrollTo = {};
  if (CaretVisibleIs()) {
    CellToScrollTo = GetHCaretCursor().Position();
    if (!CellToScrollTo)
      CellToScrollTo = GetActiveCell();
  }
  if (!CellToScrollTo)
...
void Worksheet::OnMouseRightDown(wxMouseEvent &event) {
  ...
  if (GetDocumentCellPointers().GetSelectionStart()) {
    ...
    if (GetDocumentCellPointers().GetSelectionStart()->GetType() ==
        MC_TYPE_GROUP) { // a selection of groups
      if (downx <= m_configuration->GetCellBracketWidth() + 3) {
        wxRect rectStart = GetDocumentCellPointers().GetSelectionStart()->GetRect();
        wxRect rectEnd = GetDocumentCellPointers().GetSelectionEnd()->GetRect();
    ...
    // SELECTION OF OUTPUT
      ...
      for (const Cell &cell : OnDrawList(GetDocumentCellPointers().GetSelectionStart().get())) {
        ...
        if (&cell == GetDocumentCellPointers().GetSelectionEnd())
          break;
      }
    }
  }
...
void Worksheet::OnMouseLeftInGcCell(wxMouseEvent &WXUNUSED(event),
  ...
  if ((clickedInGC->GetPrompt()) &&
    ...
    GetDocumentCellPointers().SetSelectionEnd(clickedInGC->GetPrompt());
    GetDocumentCellPointers().SetSelectionStart(GetDocumentCellPointers().GetSelectionEnd());
...
void Worksheet::OnMouseLeftDown(wxMouseEvent &event) {
  ...
  if (GetHCaretCursor().SelectionStart() && event.ShiftDown()) {
    ...
    m_down = wxPoint(GetHCaretCursor().SelectionStart()->GetRect().GetLeft(),
                     GetHCaretCursor().SelectionStart()->GetRect().GetTop());
  ...
  if (GetHCaretCursor().Position() && event.ShiftDown()) {
    // We were selecting group cells when the shift-click happened.
    m_clickType = CLICK_TYPE_GROUP_SELECTION;
    // Set a fake starting point for the selection that is inside the cell the
    // selection started in.
    m_down = wxPoint(GetHCaretCursor().Position()->GetRect().GetLeft(),
                     GetHCaretCursor().Position()->GetRect().GetBottom() + 1);
    // Handle the mouse pointer position
    OnMouseMotion(event);
    return;
  }
...
void Worksheet::OnMouseLeftUp(wxMouseEvent &event) {
  ...
  CheckUnixCopy();
  SetFocus();
  GetViewCellPointers().ResetMouseSelectionStart();
}
...
void Worksheet::OnMouseWheel(wxMouseEvent &event) {
  ...
  if(CanAnimate())
    {
      auto *animation = GetDocumentCellPointers().GetSelectionStart().CastAs<AnimationCell *>();
...
void Worksheet::SelectGroupCells(wxPoint down, wxPoint up) {
  ...
  GetDocumentCellPointers().SetSelectionStart(nullptr);
  GetDocumentCellPointers().SetSelectionEnd(nullptr);
  ...
  for (auto &cell : OnList(startSearch)) {
    if (ytop <= cell.GetRect().GetBottom()) {
      GetDocumentCellPointers().SetSelectionStart(&cell);
      break;
    }
  }

  // find out the group cell the selection ends in
  GroupCell *endSearch = GetDocumentCellPointers().GetSelectionStart().CastAs<GroupCell *>();
  if (!endSearch) endSearch = GetTree();
  for (auto &cell : OnList(endSearch)) {
    rect = cell.GetRect();
    if (ybottom < rect.GetTop()) {
      GetDocumentCellPointers().SetSelectionEnd(cell.GetPrevious());
      break;
    }
  }
  if (GetDocumentCellPointers().GetSelectionStart() && !GetDocumentCellPointers().GetSelectionEnd())
    GetDocumentCellPointers().SetSelectionEnd(GetLastCellInWorksheet());
  ...
  if (GetDocumentCellPointers().GetSelectionStart()) {
    ...
    GetHCaretCursor().SetPosition(GetLastCellInWorksheet());
  }

  if (down.y > up.y) {
    GetHCaretCursor().SetSelectionStart(GetDocumentCellPointers().GetSelectionStart().CastAs<GroupCell *>());
    GetHCaretCursor().SetSelectionEnd(GetDocumentCellPointers().GetSelectionEnd().CastAs<GroupCell *>());
  } else {
    GetHCaretCursor().SetSelectionStart(GetDocumentCellPointers().GetSelectionEnd().CastAs<GroupCell *>());
    GetHCaretCursor().SetSelectionEnd(GetDocumentCellPointers().GetSelectionStart().CastAs<GroupCell *>());
  }
  SetSelection(GetDocumentCellPointers().GetSelectionStart(), GetDocumentCellPointers().GetSelectionEnd());
}
...
void Worksheet::ClickNDrag(wxPoint down, wxPoint up) {
  const Cell *selectionStartOld = GetDocumentCellPointers().GetSelectionStart(),
    *selectionEndOld = GetDocumentCellPointers().GetSelectionEnd();
  wxRect rect;
  ...
  case CLICK_TYPE_INPUT_SELECTION:
    if (!GetViewCellPointers().MouseSelectionStart())
      return;

    {
      rect = GetViewCellPointers().MouseSelectionStart()->GetRect();
      ...
      if (!rect.Inflate(m_configuration->GetGroupSkip(),
        ...
        GetHCaretCursor().Deactivate();
        ClearSelection();
        SetActiveCell(GetViewCellPointers().MouseSelectionStart());
        ...
        selectionStartOld = GetDocumentCellPointers().GetSelectionStart();
        selectionEndOld = GetDocumentCellPointers().GetSelectionEnd();
      }
      break;
    }
  ...
  if ((selectionStartOld != GetDocumentCellPointers().GetSelectionStart()) ||
      (selectionEndOld != GetDocumentCellPointers().GetSelectionEnd()))
    RequestRedraw();
}
...
bool Worksheet::Copy(bool astext) const {
  ...
  if (!astext && GetDocumentCellPointers().GetSelectionStart()->GetType() == MC_TYPE_GROUP)
    return CopyCells();

  // If the selection is IMAGE or ANIMATION, copy it to clipboard
  // as image.
  if (GetDocumentCellPointers().GetSelectionStart() == GetDocumentCellPointers().GetSelectionEnd() &&
      (GetDocumentCellPointers().GetSelectionStart()->GetType() == MC_TYPE_IMAGE ||
       GetDocumentCellPointers().GetSelectionStart()->GetType() == MC_TYPE_SLIDE)) {
    GetDocumentCellPointers().GetSelectionStart()->CopyToClipboard();
    return true;
  }
...
bool Worksheet::CopyTeX() const {
  ...
  Cell *const start = GetDocumentCellPointers().GetSelectionStart();
  bool inMath = false;
  wxString s;
...
void Worksheet::DeleteSelection() {
  DeleteRegion(GetDocumentCellPointers().GetSelectionStart()->GetGroup(),
               GetDocumentCellPointers().GetSelectionEnd()->GetGroup());
  TreeUndo_ClearRedoActionList();
  GetDocumentCellPointers().SetSelectionStart(nullptr);
  GetDocumentCellPointers().SetSelectionEnd(nullptr);
...
bool Worksheet::OpenQuestionCaret(const wxString &txt) {
  ...
  if (!GetDocumentCellPointers().GetAnswerCell()) {
    auto answerCell =
      EditorCell::Create(group, m_configuration, MC_TYPE_INPUT);
    GetDocumentCellPointers().SetAnswerCell(answerCell.get());
  ...
  if (FollowEvaluation() || autoEvaluate)
    SetActiveCell(GetDocumentCellPointers().GetAnswerCell());
...
void Worksheet::OpenHCaret(const wxString &txt, GroupType type) {
  ...
  if (GetWorkingGroup() && QuestionPending()) {
    if ((GetActiveCell() && GetActiveCell()->GetGroup() == GetWorkingGroup()) ||
        (GetHCaretCursor().Position() &&
         GetHCaretCursor().Position() == GetWorkingGroup()->GetNext())) {
  ...
  if (GetActiveCell()) {
    ...
    SetHCaret(GetDocumentCellPointers().GetSelectionStart()->GetGroup());
  ...
  if (GetHCaretCursor().Position()) {
    while (IsLesserGCType(type, GetHCaretCursor().Position()->GetGroupType())) {
  ...
  InsertGroupCells(std::move(group), GetHCaretCursor().Position());
  // activate the editor
  SetActiveCell(editable);
...
void Worksheet::QuestionAnswered() {
  ...
  GetDocumentCellPointers().ClearAnswerCell();
...
void Worksheet::OnCharInActive(wxKeyEvent &event) {
  ...
  if ((event.GetKeyCode() == WXK_UP || event.GetKeyCode() == WXK_PAGEUP) &&
    ...
    if (event.ShiftDown()) {
      ...
      GetHCaretCursor().SetPosition(GetDocumentCellPointers().GetSelectionStart().CastAs<GroupCell *>());
      GetHCaretCursor().SetSelectionEnd(GetDocumentCellPointers().GetSelectionStart().CastAs<GroupCell *>());
      GetHCaretCursor().SetSelectionStart(GetDocumentCellPointers().GetSelectionEnd().CastAs<GroupCell *>());
  ...
  if ((event.GetKeyCode() == WXK_DOWN || event.GetKeyCode() == WXK_PAGEDOWN) &&
    ...
    if (event.ShiftDown()) {
      ...
      GetHCaretCursor().SetPosition(start);
      GetHCaretCursor().SetSelectionStart(start);
      GetHCaretCursor().SetSelectionEnd(end);
...
void Worksheet::SelectWithChar(int ccode) {
  ...
  if (!GetHCaretCursor().SelectionStart() || !GetHCaretCursor().SelectionEnd()) {
    if (GetHCaretCursor().Position())
      ...
      GetHCaretCursor().SetSelectionAnchors(GetTree(), GetTree());

    if (!GetHCaretCursor().SelectionStart())
      return;

    if (ccode == WXK_DOWN && GetHCaretCursor().Position() &&
        GetHCaretCursor().SelectionStart()->GetNext()) {
      GetHCaretCursor().SetSelectionAnchors(GetHCaretCursor().SelectionStart()->GetNext(), GetHCaretCursor().SelectionStart()->GetNext());
      while (GetHCaretCursor().SelectionStart() &&
             GetHCaretCursor().SelectionStart()->GetMaxDrop() == 0 &&
             GetHCaretCursor().SelectionStart()->GetNext())
        GetHCaretCursor().SetSelectionAnchors(GetHCaretCursor().SelectionStart()->GetNext(), GetHCaretCursor().SelectionStart()->GetNext());
    }
  } else if (ccode == WXK_UP) {
    if (KeyboardSelectionStart() &&
        GetHCaretCursor().SelectionEnd() ==
      ...
      if (prev) {
        if ((GetHCaretCursor().Position()) &&
            GetHCaretCursor().Position()->GetNext() == GetHCaretCursor().SelectionEnd())
          GetHCaretCursor().SetSelectionStart(prev);
        GetHCaretCursor().SetSelectionEnd(prev);
      }
      if (GetHCaretCursor().SelectionEnd())
        ScheduleScrollToCell(GetHCaretCursor().SelectionEnd(), false);
    }
  } else {
    ...
    if (KeyboardSelectionStart() &&
      ...
      GroupCell *nxt = GetHCaretCursor().SelectionEnd()->GetNext();
      while (nxt && nxt->GetMaxDrop() == 0)
        nxt = nxt->GetNext();

      if (nxt) {
        if (GetHCaretCursor().Position() == GetHCaretCursor().SelectionEnd())
          GetHCaretCursor().SetSelectionStart(nxt);
        GetHCaretCursor().SetSelectionEnd(nxt);
      }
      if (GetHCaretCursor().SelectionEnd())
        ScheduleScrollToCell(GetHCaretCursor().SelectionEnd(), false);
    }
  ...
  if (GetHCaretCursor().SelectionStart() && GetHCaretCursor().SelectionEnd()) {
    // GetHCaretCursor().SelectionStart() can be above or below GetHCaretCursor().SelectionEnd()
    if (GetHCaretCursor().SelectionStart()->GetCurrentY() <
        GetHCaretCursor().SelectionEnd()->GetCurrentY()) {
      SetSelection(GetHCaretCursor().SelectionStart(), GetHCaretCursor().SelectionEnd());
    } else {
      SetSelection(GetHCaretCursor().SelectionEnd(), GetHCaretCursor().SelectionStart());
    }
  }
  RequestRedraw();
}
  ...
  if (editor && (m_configuration->ShowCodeCells() ||
    ...
    if (up) {
      if (!GetHCaretCursor().Position())
        ...
        if (GetHCaretCursor().Position()->GetNext())
          SetHCaret(GetHCaretCursor().Position()->GetNext());
        else
          SetHCaret(GetLastCellInWorksheet());
      }
    } else
      SetHCaret(GetHCaretCursor().Position()->GetPrevious());
  }
  RequestRedraw();
}
...
void Worksheet::OnCharNoActive(wxKeyEvent &event) {
  ...
  if (GetDocumentCellPointers().GetSelectionStart() &&
      GetDocumentCellPointers().GetSelectionStart()->GetType() == MC_TYPE_SLIDE &&
      ccode == WXK_SPACE) {
    GetDocumentCellPointers().GetSelectionStart().CastAs<AnimationCell *>()->AnimationRunning(
                                                                                !GetDocumentCellPointers().GetSelectionStart().CastAs<AnimationCell *>()
                                                                                ->AnimationRunning());
    return;
  }
  ...
  case WXK_HOME:
    ...
    if (event.CmdDown()) {
      ...
      if (event.ShiftDown()) {
        ...
        GetHCaretCursor().SetSelectionStart(oldCell);
        GetHCaretCursor().SetSelectionEnd(GetTree());
      }
      ScrolledAwayFromEvaluation();
    }
    break;
  ...
  case WXK_END:
    ...
    if (event.CmdDown()) {
      ...
      if (event.ShiftDown()) {
        ...
        GetHCaretCursor().SetSelectionStart(oldCell);
        GetHCaretCursor().SetSelectionEnd(GetLastCellInWorksheet());
      }
      ScrolledAwayFromEvaluation();
    }
    break;
  ...
  case WXK_BACK:
    if (GetHCaretCursor().Position()) {
      SetSelection(GetHCaretCursor().Position());
      RequestRedraw();
      GetHCaretCursor().Deactivate();
      return;
    }
  ...
  case WXK_NUMPAD_DELETE:
    if (!GetHCaretCursor().Position()) {
      ...
      ScrolledAwayFromEvaluation();
    } else if (GetHCaretCursor().Position()->GetNext()) {
      SetSelection(GetHCaretCursor().Position()->GetNext());
      GetHCaretCursor().Deactivate();
      return;
    }
    break;
  ...
  case WXK_LEFT:
    ...
    if (GetHCaretCursor().IsActive()) {
      if (GetDocumentCellPointers().GetSelectionStart()) {
        if (event.CmdDown()) {
          GroupCell *tmp =
            GetDocumentCellPointers().GetSelectionStart().CastAs<GroupCell *>();
          if (tmp->GetPrevious()) {
            do
              {
                tmp = tmp->GetPrevious();
            ...
            if (GetHCaretCursor().Position() && GetHCaretCursor().Position()->GetEditable())
              SelectEditable(tmp->GetEditable(), false);
          }
        } else
          SetHCaret(GetDocumentCellPointers().GetSelectionStart()->GetGroup()->GetPrevious());
        ...
        if (event.CmdDown()) {
          ...
          if (GetHCaretCursor().Position()->GetEditable())
            SelectEditable(GetHCaretCursor().Position()->GetEditable(), false);
        }
      }
  ...
  case WXK_RIGHT:
    ...
    ScrolledAwayFromEvaluation(true);
    if (GetHCaretCursor().IsActive()) {
      if (GetDocumentCellPointers().GetSelectionEnd()) {
        if (event.CmdDown()) {
          GroupCell *tmp = GetDocumentCellPointers().GetSelectionEnd().CastAs<GroupCell *>();
          if (tmp->GetNext()) {
            do
              {
                tmp = tmp->GetNext();
        ...
        if (event.CmdDown()) {
          ...
          SelectEditable(GetHCaretCursor().Position()->GetNext()->GetEditable(), true);
      } else if (GetTree() && !GetHCaretCursor().Position()) {
        SelectEditable(GetTree()->GetEditable(), true);
      }

    } else if (GetDocumentCellPointers().GetSelectionEnd())
      SetHCaret(GetDocumentCellPointers().GetSelectionEnd()->GetGroup());
...
void Worksheet::StepAnimation(int change) {
  if (GetSelectionStart() && GetSelectionStart() == GetSelectionEnd() &&
    ...
    if (change != 0) {
      ...
      wxRect rect = GetDocumentCellPointers().GetSelectionStart()->GetRect();
      RequestRedraw(rect);
...
void Worksheet::OnTimer(wxTimerEvent &event) {
    ...
    if (m_blinkDisplayCaret) {
      ...
      if (GetActiveCell()) {
        ...
        if (!GetHCaretCursor().Position()) {
          ...
          rect = GetHCaretCursor().Position()->GetRect();
          int caretY =
  ...
  default: {
    ...
    Cell *const cell = GetViewCellPointers().GetCellForTimerId(event.GetId());
...
wxSize Worksheet::CopyToFile(const wxString &file) const {
  if (GetDocumentCellPointers().GetSelectionStart() &&
      ...
      (GetDocumentCellPointers().GetSelectionStart()->GetType() == MC_TYPE_IMAGE ||
       GetDocumentCellPointers().GetSelectionStart()->GetType() == MC_TYPE_SLIDE)) {
    return GetDocumentCellPointers().GetSelectionStart().CastAs<ImgCellBase *>()->ToImageFile(
                                                                                file);
  } else {
...
void Worksheet::TOCdnd(GroupCell *dndStart, GroupCell *dndEnd) {
  ...
  for (auto &src : OnList(GetDocumentCellPointers().GetSelectionStart().get())) {
    copy.Append(src.Copy(dynamic_cast<GroupCell *>(&src)));
    if (&src == GetDocumentCellPointers().GetSelectionEnd())
      break;
  }
...
void Worksheet::OnDoubleClick(wxMouseEvent &event) {
  ...
  if (GetActiveCell())
    ...
    const GroupCell *parent = GetDocumentCellPointers().GetSelectionStart()->GetGroup();
...
bool Worksheet::ActivateInput(int direction) {
  ...
  GroupCell *tmp = {};
  if (GetDocumentCellPointers().GetSelectionStart())
    tmp = GetDocumentCellPointers().GetSelectionStart()->GetGroup();
  else if (GetActiveCell()) {
...
void Worksheet::AddRestToEvaluationQueue() {
  GroupCell *start = {};
  if (HasCellsSelected())
    start = GetDocumentCellPointers().GetSelectionStart()->GetGroup();
...
void Worksheet::AddSelectionToEvaluationQueue() {
  AddSelectionToEvaluationQueue(
                                GetDocumentCellPointers().GetSelectionStart().CastAs<GroupCell *>(),
                                GetDocumentCellPointers().GetSelectionEnd().CastAs<GroupCell *>());
}
...
void Worksheet::AddDocumentTillHereToEvaluationQueue() {
  FollowEvaluation(true);
  const GroupCell *stop = GetHCaretCursor().Position();
  if(!GetHCaretCursor().IsActive()) stop = nullptr;
  if (!stop) {
    if (!GetActiveCell())
      return;
...
void Worksheet::ScrollToCellIfNeeded() {
  if (!GetViewCellPointers().ScrollToCellScheduled())
    return;
  GetViewCellPointers().SetScrollToCellScheduled(false);

  RecalculateIfNeeded();

  const Cell *cell = GetViewCellPointers().CellToScrollTo();
...
void Worksheet::SetSelection(Cell *start, Cell *end) {
  if (GetDocumentCellPointers().GetSelectionStart() != start ||
      GetDocumentCellPointers().GetSelectionEnd() != end)
    RequestRedraw();
  GetDocumentCellPointers().SetSelectionStart(start);
...
void Worksheet::PasteFromClipboard() {
  ...
  if (!cells) {
    if (GetActiveCell()) {
      ...
      if (GetHCaretCursor().IsActive() && (wxTheClipboard->IsSupported(wxDF_TEXT) ||
                             wxTheClipboard->IsSupported(wxDF_UNICODETEXT))) {
...
void Worksheet::MergeCells() {
  ...
  for (auto &tmp :
         OnList(GetDocumentCellPointers().GetSelectionStart().CastAs<GroupCell *>())) {
    if (newcell.Length() > 0)
      newcell += wxS("\n");
  ...
  GroupCell *selStart = GetDocumentCellPointers().GetSelectionStart().CastAs<GroupCell *>();
  EditorCell *editor = selStart->GetEditable();
  editor->SetValue(newcell);

  GetDocumentCellPointers().SetSelectionStart(selStart->GetNext());
  DeleteSelection();
...
GroupCell *Worksheet::GetHCaret() {
  if (GetHCaretCursor().IsActive())
    return GetHCaretCursor().Position();

  if (GetActiveCell())
    return GetActiveCell()->GetGroup();

  if (GetDocumentCellPointers().GetSelectionStart())
    return GetDocumentCellPointers().GetSelectionStart()->GetGroup();
...
void Worksheet::SetHCaret(GroupCell *where) {
  ...
  GetHCaretCursor().Activate();
  if (GetHCaretCursor().Position() != where) {
    GetHCaretCursor().SetPosition(where);
...
void Worksheet::RemoveAllOutput() {
  if (HasCellsSelected()) {
    ...
    if (GetDocumentCellPointers().GetSelectionStart()->GetType() != MC_TYPE_GROUP)
      ClearSelection();
  }
...
wxString Worksheet::GetInputAboveCaret() {
  if (!GetHCaretCursor().IsActive() || !GetHCaretCursor().Position())
    return {};

  const EditorCell *editor = GetHCaretCursor().Position()->GetEditable();
...
WorksheetSearch::SearchStart Worksheet::FindStart(bool down) {
  int starty;
  if (down)
    starty = 0;
  else {
  ...
  WorksheetSearch::SearchStart start;
  if (GetActiveCell()) {
    pos = GetActiveCell()->GetGroup();
    start.m_inEditor = true;
  } else if (GetSelectionStart()) {
    pos = GetSelectionStart()->GetGroup();
    start.m_atCell = GetSelectionStart();
  } else if (GetHCaretCursor().IsActive()) {
...
bool Worksheet::ApplyFindResult(const WorksheetSearch::SearchTarget &target,
  ...
  if (target.m_part == WorksheetSearch::MatchPart::Editor) {
...
bool Worksheet::FindNext(const wxString &str, bool down, bool ignoreCase,
  ...
  const WorksheetSearch::SearchStart start = FindStart(down);
  ...
  return ApplyFindResult(
    WorksheetSearch::FindNextTarget(GetTree(), GetLastCellInWorksheet(),
                                    start, down, matcher, searchInInput,
                                    searchInOutput),
    warn);
}
...
bool Worksheet::FindNext_Regex(const wxString &str, const bool &down,
  ...
  if (start.m_group->GetEditable())
    start.m_group->GetEditable()->SearchStartedHere(
      start.m_group->GetEditable()->GetCaretPosition());
...
bool Worksheet::CaretVisibleIs() {
  if (GetHCaretCursor().IsActive()) {
    int y = -1;
    if (GetHCaretCursor().Position())
      y = GetHCaretCursor().Position()->GetCurrentY();
...
bool Worksheet::ScrollToCaretIfNeeded() {
  ...
  RecalculateIfNeeded();
  if (GetHCaretCursor().IsActive()) {
    ScheduleScrollToCell(GetHCaretCursor().Position(), false);
  } else {
    if (GetActiveCell()) {
      wxPoint point = GetActiveCell()->PositionToPoint();
...
void Worksheet::OpenNextOrCreateCell() {
  if (GetHCaretCursor().Position() && GetHCaretCursor().Position()->GetNext()) {
    SetSelection(GetHCaretCursor().Position());
    ActivateNextInput();
  } else
    OpenHCaret();
...
void Worksheet::SelectGroupCell(GroupCell *cell) {
  ...
  if (cell) {
    ...
    GetHCaretCursor().SetSelectionEnd(cell);
    GetHCaretCursor().SetSelectionStart(cell);
  }
}
