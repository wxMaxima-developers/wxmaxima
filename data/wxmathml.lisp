...
(defvar wxArtDir "" "The location wxMaxima looks for icons and images in")
...
(defun $wxbuild_info ()
  (let ((year (sixth cl-user:*maxima-build-time*))
    ...
    (format t "~%~%wxMaxima's idea of the directory layout is:~%Icon dir: ~a~%" wxArtDir)
    (format t "User configuration dir: ~a~%" wxUserConfDir)
...
(defun wxxml-stripdollar (sym &aux pname)
  ...
  (setq pname (maybe-invert-string-case (wxxml-fix-string (symbol-name sym))))
