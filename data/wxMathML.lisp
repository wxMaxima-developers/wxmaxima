  ...
  (defun subscriptp (x)
	...
	(let* ((sub (subseq name (+ pos 1)))
	  ...
	  (when (and (> (length sub-var) 0)
	    ...
	    (format nil  "<i altCopy=\"~{~a~}\"><mrow>~a</mrow><mrow>~a</mrow></i>"
		    (mstring x)
	    ...
	    (t
	     ...
	     (cond ((null exponent)
		   ...
		   (t
		    ...
		    (format nil
			    "<mrow><n>~{~c~}</n><h>*</h><e><n>10</n><n>~{~c~}</n></e></mrow>"
			    firstpart exponent)))))))
    ...
    (let((nc (eq (caar x) 'mncexpt)))
      (setq l (wxxml (cadr x) (append l (if nc
	    ...
	    r (if (mmminusp (setq x (nformat (caddr x))))
		  ;; the change in base-line makes parens unnecessary
		  (wxxml (cadr x) '("</mrow><mrow><v>-</v>")
			 (cons "</mrow></e>" r) 'mminus 'mminus)
		(if (and (integerp x) (< x 10))
		    (wxxml x (list "</mrow>")
			   (cons "</e>" r) 'mparen 'mparen)
		  (wxxml x (list "</mrow><mrow>")
			 (cons "</mrow></e>" r) 'mparen 'mparen)
		  )))
      (append l r)))
      ...
      (case (fifth x)
	    ($plus
	     (append l `("<lm><fnm>lim</fnm><mrow>"
			 ,@subfun "<v>+</v></mrow><mrow>"
			 ,@s1 "</mrow></lm>") r))
	    ($minus
	     (append l `("<lm><fnm>lim</fnm><mrow>"
			 ,@subfun "<t>-</t></mrow><mrow>"
			 ,@s1 "</mrow></lm>") r))
	    (otherwise
	     (append l `("<lm><fnm>lim</fnm><mrow>"
			 ,@subfun "</mrow><mrow>"
  ...
  (defun wxxml-d-abbrev (x)
	...
	((difflist (cddr x))
	 ...
	 (fun (wxxml (cadr x) nil nil 'mparen 'mparen)))
      (append '("<i d=\"1\"><mrow>") fun '("</mrow>")
	      '("<mrow>") (wxxml-d-abbrev-subscript vars ords) '("</mrow></i>"))))
  ...
  (defun wxxml-orthopoly (x l r)
    (let* ((fun-name (caar x))
      ...
      (append l
		...
		(list (format nil "<i><fnm>~a</fnm><mrow>" (car disp-name))))
    ...
    (cond ((and $pdiff_uses_prime_for_derivatives (eq 3 (length x)))
	  ...
	  ((and $pdiff_uses_named_subscripts_for_derivatives
	   ...
	   (let ((n (cddr x))
	     ...
	     (append (append l '("<i><mrow>"))
		     (wxxml (cadr x) nil nil lop rop)
		     (list "</mrow><mrow>") p (list "</mrow></i>") r)))
	  (t
	   (append (append l '("<i><mrow>"))
		   (wxxml (cadr x) nil nil lop rop)
		   (list "</mrow><mrow>(")
