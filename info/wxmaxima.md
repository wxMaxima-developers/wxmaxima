...
A matrix cannot be broken into lines the way a long sum can. So by default wxMaxima leaves out the middle rows and columns of a matrix that is too large for the window and marks the gap with `⋯`, `⋮` and `⋱`, the way a large matrix is written by hand. The first and last rows and columns always stay visible, and hovering the mouse over the matrix tells which rows and columns are not shown. The option _Matrices too large for the window_ in the _Worksheet_ tab of the configuration dialog can instead draw such a matrix in full, which makes the whole worksheet scroll sideways, or show it in a box the size of the window, with scrollbars of its own; there the mouse wheel still scrolls the worksheet, and such a matrix is printed with its middle rows and columns left out. All of this affects only what is shown on screen and printed: copying, saving and exporting the matrix always include every entry.
